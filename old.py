...
class Database():
    ...
    def connect(self,host,database,user,password):
        try:
            ...
            logger.info(f"Database {database} connection established successfully.")
            return self.connection 
        except Exception as e:
            logger.error(f"Error connecting to the database {database}: {e}")
            return None
    ...
    def connect_to_olap_tmp_db(self):
    ...
    def load_to_tmp_table(self,table_name,file_name):
        try:
            if self.stg_connection: 
                with self.stg_connection.cursor() as cursor:
                    file_path = f"{Variables.csv_output_path}/{file_name}"
    def load_to_stg_table(self, table_name, file_name):
        try:
            if self.stg_connection:
                with self.stg_connection.cursor() as cursor:
                    file_path = f"{Variables.csv_output_path}/{file_name}"
